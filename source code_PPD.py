    ...
    if epds_col and age_col and days_col:
        ...
        if len(X) == 0 or len(y) == 0:
            ...
            if high_risk_df.empty:
                ...
                SENDER_EMAIL = "ppdwellbeings@gmail.com"
                CLINICAL_TEAM_EMAILS = ["ppd.alerts.system@gmail.com"]
                ...
                def gmail_authenticate():
                    ...
                    # ❗ NO impersonation (Gmail personal accounts do not support domain delegation)
                    return build("gmail", "v1", credentials=creds, cache_discovery=False)

                def send_gmail_alert_html(df, service):
                    ...
                    message["to"] = ", ".join(CLINICAL_TEAM_EMAILS)
                    message["from"] = f"Postpartum Alerts <{SENDER_EMAIL}>"
                    message["subject"] = subject
                    message.attach(MIMEText(html_body, "html"))

                    raw = base64.urlsafe_b64encode(message.as_bytes()).decode()
                    service.users().messages().send(userId="me", body={"raw": raw}).execute()
                    st.success("📧 Gmail alert sent successfully to Clinical Team!")

                try:
                    gmail_service = gmail_authenticate()
                    send_gmail_alert_html(high_risk_df, gmail_service)
                except Exception as e:
